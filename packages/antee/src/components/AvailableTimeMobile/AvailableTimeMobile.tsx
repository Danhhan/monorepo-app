...
import NotimeFree from './components/NotimeFree';
import TimeSlider from './components/TimeSlider';

const maximumDisplayedItems = 12;

export type AvailableTimeMobileProps = {
  date: string;
  successHandle?: () => void;
  handleToggle?: () => void;
  teacherId: number;
  ...
  dataCampain?: {};
  isVisiableModalMobile: boolean;
  closeModal?: any;
  handleSelectime: (data: any) => void;
  timeBottomselected: any;
};

const AvailableTimeMobile: React.FC<AvailableTimeMobileProps> = ({
  date,
  successHandle,
  isAcceptTrial,
  teacherId,
    ...
    <>
      {isAuthenticated ? (
          ...
          {isLoading || isFetching ? (
            ...
            <>
              <TimeSlider
                sliderRef={sliderRef}
                settings={settings}
                arraySplited={arraySplited || []}
                handleSelectTime={handleSelectTime}
                timeSelected={timeSelected || null}
              />
              <Spacer />

              <TimeSelected
                dataLenght={data?.data?.data?.length}
                maximumDisplayedItems={maximumDisplayedItems}
        ...
        <>
          <ProvideLearning
            dataCampain={dataCampain || {}}
            isVisiableModal={isVisiableModalMobile}
...
export default AvailableTimeMobile;
