...
const AvailableTimeTrial: React.FC<{}> = () => {
  ...
  useEffect(() => {
    ...
    if (localDaysOfWeek.length > 0 && role === ROLE_TRIAL) {
      dispatch(
    ...
    if (newDurationList.length > 0 && role === ROLE_TRIAL) {
      dispatch(
  ...
  return (
    <>
      <Spacer />
      <DaysOfWeek teachingTime={DURATION_TRIAL} />
      <Spacer />
      {findDayOfWeek && (
        <>
