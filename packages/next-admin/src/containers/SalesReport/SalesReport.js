...
import { getReportsDaily, GET_REPORTS_DAILY } from 'services/salesReport';
...
const SalesReport = () => {
  ...
  const [items, setItems] = useState(UTM_SOURCE_LIST);
  ...
  function updateItem(index) {
    ...
    setItems(newItems);
    const _newUtmList = items.map(item => item.checked === 'on' && item.value); // get value
    const filterUtm = _newUtmList.filter(item => item); // filter item
    const flatUtmList = filterUtm.reduce((utm, value) => utm.concat(value), []); // flat array
    setUtmList(flatUtmList);
  }
  return (
    <Page
      className="mx-auto"
          ...
          <PageHeaderSection>
            ...
            {items.map((item, index) => (
              <FilterButton
                ...
                hasActiveFilters={item.checked === 'on'}
              >
                <FormattedMessage
                  defaultMessage="{name}"
        ...
        >
          <div>
            {error ? (
              ...
              >
                {data?._data.reports.map(team => (
                  <FlexItem key={team.order} style={{ width: 'fit-content' }}>
              ...
              <div className="flex justify-center flex-col items-center py-40">
                ...
                <AntoreeCustomizeLoading antoreeSize="s" />
              </div>
            )}
          </div>
          <Spacer />
